...
create index if not exists photos_gallery_idx on public.photos(is_gallery_visible, created_at desc);
alter table public.photo_batches enable row level security;
alter table public.photos enable row level security;
insert into storage.buckets (id,name,public,file_size_limit,allowed_mime_types)
values ('daac-photos','daac-photos',false,26214400,array['image/jpeg','image/png','image/webp','image/heic','image/heif'])
