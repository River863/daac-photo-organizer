...
def app_script(action, **payload):
    ...
    if not url or not token:
        raise RuntimeError("The Google Drive connection has not been configured yet.")
...
def render_album():
    ...
    if featured:
        cards = "".join("<figure><img src='data:image/jpeg;base64,{0}'><figcaption>{1}</figcaption></figure>".format(p["thumbnail"], p.get("albumName", "DAAC")) for p in featured)
        st.markdown(f"<div class='photo-strip'>{cards}{cards}</div>", unsafe_allow_html=True)
    else:
    ...
    if not albums:
        st.info("No photo albums have been added yet.")
    for album in albums:
        with st.expander(f"{album['name']}  ·  {album['count']} photos"):
            ...
            if not photos:
                ...
                for i, photo in enumerate(photos):
                    columns[i % len(columns)].image(image_bytes(photo), use_container_width=True)
...
def render_upload():
    ...
    if any(photo.size > 15 * 1024 * 1024 for photo in photos):
        st.error("Each photo must be 15 MB or smaller.")
        return
    ...
    for batch in batches:
        with st.container(border=True):
            ...
            if approve.button("Approve into event album", type="primary", use_container_width=True, key=f"approve_{batch['id']}"):
                try:
                    app_script("approve", batchId=batch["id"], category=category)
                    st.rerun()
                except Exception as exc:
                    st.error(str(exc))
